...
```{r densityPlot, fig.cap = 'Visualising the variance in x.'}
...
variance!  In R `sd(x) * sd(x)` = `var(x)` = `r sd(x) * sd(x)`

Unlike the variance, the standard deviation can be visualized.

```{r sdPlot, fig.cap = 'Standardized spread: Standard deviation and mean of x.'}
...
```{r covPlot, fig.cap = 'Visualizing covariance: Scatter plot of x and y'}
...
```{r regPlot, fig.cap = 'Linear regression line'}
...
```{r scaleRegPlot, fig.cap = 'Scaled variables and linear regression line'}
